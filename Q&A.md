The core principle of Kubernetes that involves declaring a desired state and letting the system work to achieve it is Declarative State Management.
...
On the front it reads: "What command allows a non-root user in a Docker container to bind a process to privileged ports like port 80?"
The answer on the back reads: "setcap."
...
The Solution:  and Linux Capabilities
...
On the front it reads: "What is the fundamental difference between a Docker image and a Docker container from a file-system perspective?"
The answer on the back reads: "An image is a collection of read-only layers, while a container adds a thin, writable layer on top of the image layers."
...
On the front it reads: "Which Linux kernel feature is used by Docker to limit and control the amount of CPU, memory, and I/O a container can use?"
The answer on the back reads: "Control Groups (cgroups)."
...
On the front it reads: "When you run a container with -p 8080:80, Docker manipulates the host's firewall rules, typically using what tool?"
The answer on the back reads: "iptables."
...
On the front it reads: "For communication between containers on a single host, you would typically use a _____ network in Docker."
The answer on the back reads: "bridge"
...
On the front it reads: "For communication between containers running across multiple hosts in a cluster, you should use an _____ network in Docker."
The answer on the back reads: "overlay"
